...
describe('API Utilities', () => {
  describe('extractIdFromUrl', () => {
    ...
    it('should extract ID from generation URL', () => {
      expect(extractIdFromUrl('https://pokeapi.co/api/v2/generation/3/')).toBe(3);
    });
  });

  describe('getGenerationName', () => {
    it('should format generation I', () => {
      expect(getGenerationName('https://pokeapi.co/api/v2/generation/1/')).toBe('Generation I');
    });

    it('should format generation V', () => {
      expect(getGenerationName('https://pokeapi.co/api/v2/generation/5/')).toBe('Generation V');
    });

    it('should format generation IX', () => {
      expect(getGenerationName('https://pokeapi.co/api/v2/generation/9/')).toBe('Generation IX');
    });
  });
});
...
describe('Filter Utilities', () => {
  ...
  describe('applyFilters', () => {
    it('should apply type and generation filters together', () => {
      const result = applyFilters(mockPokemon, { type: 'fire', generation: '1' });
      expect(result).toHaveLength(1);
      expect(result[0].name).toBe('charizard');
    });
